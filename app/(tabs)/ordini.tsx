...
export default function OrdiniScreen() {
  ...
  const handleOrderPress = (order: any) => {
    Alert.alert(
      `${order.name}`,
      `Quantità: ${order.quantity}\nPrezzo: €${order.price.toFixed(2)}\nStato: ${getStatusText(order.status || 'pending')}`
    );
  };

  const renderOrderItem = ({ item }: { item: any }) => (
    <TouchableOpacity
      style={[styles.orderItem, { backgroundColor: cardBg }]}
      onPress={() => handleOrderPress(item)}
    >
      <ThemedView style={styles.orderHeader}>
        <ThemedText type="subtitle" style={styles.pizzaName}>{item.name}</ThemedText>
        <ThemedText style={{ color: getStatusColor(item.status || 'pending'), fontWeight: '600' }}>
          {getStatusText(item.status || 'pending')}
        </ThemedText>
      </ThemedView>

      <ThemedView style={styles.orderDetails}>
        <ThemedText style={[styles.quantity, { color: mutedText }]}>Quantità: {item.quantity}</ThemedText>
  ...
  return (
    <ThemedView style={styles.container}>
      <ThemedView style={styles.header}>
        <ThemedText type="title" style={styles.title}>I tuoi ordini</ThemedText>
        <TouchableOpacity style={styles.newOrderButton} onPress={() => router.push('/(tabs)')}>
          <IconSymbol size={20} name="plus" color="#007AFF" />
          <ThemedText style={styles.newOrderText}>Nuovo ordine</ThemedText>
        </TouchableOpacity>
      </ThemedView>
...
const styles = StyleSheet.create({
  ...
  newOrderButton: { flexDirection: 'row', alignItems: 'center', gap: 5, padding: 8 },
  newOrderText: { color: '#007AFF', fontSize: 16, fontWeight: '600' },
  ordersList: { padding: 20 },
  orderItem: { borderRadius: 12, padding: 16, marginBottom: 12, shadowColor: '#000', shadowOpacity: 0.1, shadowOffset: { width: 0, height: 2 }, shadowRadius: 4, elevation: 3 },
  orderHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
  pizzaName: { fontSize: 18, fontWeight: '600', flex: 1 },
  ...
  emptyTitle: { fontSize: 20, fontWeight: '600', marginTop: 20, marginBottom: 10 },
  emptyText: { fontSize: 16, textAlign: 'center', color: '#666', lineHeight: 24 },
});
